...
ui <- fluidPage(theme='SUPERHERO',
  ...
  dashboardPage(
    ...
    dashboardBody(
      fluidRow(
        box(title = "Cutomer Demographic Size", width = NULL, solidHeader = TRUE, status = "primary", color = "#289259",
            fluidRow(
              box(align = "center",
                  title = NULL, 
                  width = 12,
                  status = NULL,
                  box(width = NULL, plotOutput("plot1", height = "200px")))
            ),
            #fluidRow(
             # column(width = 4,
...
### Plots  
  output$plot1 <- renderPlot({
    ...
    plot1 <- 
      ggplot(rv$dataset, aes(x = Churn, fill = Churn)) +
      geom_bar(position = "dodge", fill = c("#CD6155", "#286192"), color = c("#CD6155", "#376e37")) +
  ...
  output$plot3 <- renderPlot({
    ...
    p1 <- ggplot(rv$dataset, aes(x=Contract, fill = factor(Contract)))+
      geom_bar(position = "dodge",  fill = c("#CD6155","#D98880","#E6B0AA"), color = "#1e496d") +
     # coord_flip() +
      geom_text(aes(label = paste(round(..count../nrow(rv$dataset)*100,2), "%")),
                stat = "count", position = "dodge", vjust = 2) +
    ...
    p3 <- ggplot(rv$churn, aes(x=Contract, fill = factor(Contract)))+
     ...
     # coord_flip() +
      ...
      theme(legend.position = "none",
            ...
            panel.grid=element_blank(),
            panel.border=element_blank())
    plot3 <- grid.arrange(p1, p2, p3, nrow = 1)
    plot3
  })
  ...
  output$plot4 <- renderPlot({
    ...
    p1 <- ggplot(rv$dataset, aes(x=PaymentMethod, fill = factor(PaymentMethod)))+
      geom_bar(position = "dodge",  fill = c("#CD6155","#D98880","#E6B0AA","#F2D7D5"), color = "#1e496d") +
     # coord_flip() +
      geom_text(aes(label = paste(round(..count../nrow(rv$dataset)*100,2), "%")),
                stat = "count", position = "dodge", vjust=5) +
      labs(title = "All Customers", fill = '')+
      theme_minimal() +
      theme(legend.position = "none",
            axis.ticks=element_blank(),
   ...
   #   coord_flip() +
    ...
    #  coord_flip() +
      ...
      theme(legend.position = "none",
            ...
            panel.grid=element_blank(),
            panel.border=element_blank())
    plot4 <- grid.arrange(p1, p2, p3, nrow = 1)
    plot4
  })

  output$plot5 <- renderPlot({
    bar_fills <- c("brown", "gray")
    plot5 <- ggplot(rv$dataset, aes(tenure, fill = Churn)) +
      geom_density(aes(alpha = 1/10)) +
      scale_fill_manual(values = bar_fills,
                        guide = "none") +
      xlab('Number of Months') +
      theme_minimal()
  ...
  output$plot6 <- renderPlot({
    ...
    p1 <- ggplot(rv$dataset, aes(x=PaperlessBilling, fill = factor(PaperlessBilling)))+
      geom_bar(position = "dodge",  fill = c("brown","gray"), color = "#1e496d") +
     # coord_flip() +
      geom_text(aes(label = paste(round(..count../nrow(rv$dataset)*100,2), "%")),
                stat = "count", position = "dodge", vjust = 5) +
      labs(title = "All Customers", fill = '')+
    ...
    p2 <- ggplot(rv$stayed, aes(x=PaperlessBilling, fill = factor(PaperlessBilling)))+
      geom_bar(position = "dodge", fill = c("#E74C3C", "#7F8C8D"), color = ("#376e37")) +
     # coord_flip() +
      geom_text(aes(label = paste(round(..count../nrow(rv$stayed)*100,2), "%")),
                stat = "count", position = "dodge",vjust = 5) +
      labs(title = "Customers Stayed", fill = '')+
    ...
    p3 <- ggplot(rv$churn, aes(x=PaperlessBilling, fill = factor(PaperlessBilling)))+
      geom_bar(position = "dodge", fill = c("#CD6155", "#AAB7B8"), color = "#ba7412") +
      #coord_flip() +
      geom_text(aes(label = paste(round(..count../nrow(rv$churn)*100,2), "%")),
                stat = "count", position = "dodge", vjust = 5) +
      labs(title = "Customers Left", fill = '')+
      ...
      theme(legend.position = "none",
            ...
            panel.grid=element_blank(),
            panel.border=element_blank())
    plot6 <- grid.arrange(p1, p2, p3, nrow = 1)
    plot6
  })
  ...
  output$plot8 <- renderPlot({
    plot8 <- ggplot(rv$dataset, aes(Churn %in% c("Stayed", "Churn"))) +
      ...
      scale_color_manual(values =  c("orange", "#7F8C8D")) +
  ...
  output$plot10 <- renderPlot({
    ...
    p2 <- ggplot(rv$stayed, aes(x=InternetService, fill = factor(InternetService)))+
      geom_bar(position = "dodge",fill =c("#73C6B6","#A2D9CE","#D0ECE7"), color = ("#376e37")) +
   #   coord_flip() +
      geom_text(aes(label = paste(round(..count../nrow(rv$stayed)*100,2), "%")),
                stat = "count", position = "dodge",hjust = -0.1) +
      labs(title = "Customers Stayed", fill = '')+
  ...
  #    coord_flip() +
      ...
      theme(legend.position = "none",
            ...
            panel.grid=element_blank(),
            panel.border=element_blank())
    plot10 <- grid.arrange(p1, p2, p3, nrow = 1)
  ...
  output$plot11 <- renderPlot({
    p1 <- ggplot(rv$dataset, aes(x=PhoneServ, fill = factor(PhoneServ)))+
      ...
      #coord_flip() +
      geom_text(aes(label = paste(round(..count../nrow(rv$dataset)*100,2), "%")),
                stat = "count", position = "dodge",  vjust = 1.5,hjust=0.5) +
      labs(title = "All Customers", fill = '')+
      theme_minimal() +
      theme(legend.position = "none",
            axis.ticks=element_blank(),
    ...
    p3 <- ggplot(rv$churn, aes(x=PhoneServ, fill = factor(PhoneServ)))+
      ...
      theme(legend.position = "none",
            ...
            panel.grid=element_blank(),
            panel.border=element_blank())
    plot11 <- grid.arrange(p1, p2, p3, nrow = 1)
    plot11
  })
  output$plot12 <- renderPlot({
    ...
    p2 <- ggplot(rv$stayed, aes(x=Streaming, fill = factor(Streaming)))+
      geom_bar(position = "dodge",fill = c("#73C6B6","#A2D9CE","#D0ECE7","#E8F8F5"), color = ("#376e37")) +
      #coord_flip() +
      geom_text(aes(label = paste(round(..count../nrow(rv$stayed)*100,2), "%")),
                stat = "count", position = "dodge", vjust = 1.5,hjust=0.5) +
      labs(title = "Customers Stayed", fill = '')+
    ...
    p3 <- ggplot(rv$churn, aes(x=Streaming, fill = factor(Streaming)))+
      ...
      theme(legend.position = "none",
            ...
            panel.grid=element_blank(),
            panel.border=element_blank())
    plot12 <- grid.arrange(p1, p2, p3, nrow = 1)
    plot12
  })
}
